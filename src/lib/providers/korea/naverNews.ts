...
export async function fetchNaverNewsMentions(query: string) {
  ...
  if (!id || !secret) {
    return emptyProviderResult('naver-news', '뉴스 API 키 없음 · empty fallback', []);
  }
  const url = new URL('https://openapi.naver.com/v1/search/news.json');
  url.searchParams.set('query', query);
  url.searchParams.set('display', '5');
  url.searchParams.set('sort', 'date');
  const raw = await safeFetchJson<{ items?: Array<{ title?: string; link?: string; pubDate?: string }> }>(url.toString(), {
    headers: {
