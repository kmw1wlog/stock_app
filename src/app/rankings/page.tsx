          ...
          <div key={category.key} className="space-y-5">
            <section className="mx-5 rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-3">
                {category.items.length ? category.items.slice(0, 10).map((card, itemIndex) => <RankingRow key={`${category.key}-${card.id}`} card={card} index={itemIndex} />) : <EmptyState />}
              </div>
            </section>
            {index === 1 || index === 3 ? (
              <NativeAdCard source="rankings" slotName={`rankings_mid_${index}`} title="관심 종목을 내 MTS에서 확인하세요." description="조건식 후보와 분리된 제휴 콘텐츠입니다." />
            ) : null}
          </div>
        ))}
