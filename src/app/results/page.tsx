...
export default function ResultsPage() {
  ...
  const visibleIds = Array.from(new Set([...trackingIds, 'ecopro', 'bitcoin-leverage']));
      ...
      <div className="space-y-5 py-6">
        <header className="flex items-start justify-between px-5">
          <div className="flex gap-4">
            ...
            <div>
              <h1 className="text-4xl font-black tracking-normal">결과</h1>
              <p className="mt-2 text-sm font-semibold text-slate-500">저장한 카드와 넘긴 카드가 이후 어떻게 움직였는지 봅니다.</p>
            </div>
          </div>
        ...
        <div className="mx-5 grid grid-cols-4 overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          <button className="py-4 text-xs font-black text-slate-500">차트자리</button>
        ...
        <section className="mx-5 flex items-center justify-between rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div>
            <p className="text-xl font-black">
              이번 주 추적 중인 카드 <span className="text-[#0B63F6]">{trackingIds.length}개</span>
            </p>
            <p className="mt-2 text-sm font-bold text-slate-500">
              기준 시점 이후 단순 계산 평균 <span className="text-emerald-600">+1.28%</span>
            </p>
          </div>
          <Sparkline />
        </section>
        <section className="mx-5 rounded-3xl border border-slate-200 bg-white p-4">
          <p className="mb-3 text-sm font-black text-orange-500">놓친 카드</p>
          <div className="space-y-2">
            {missedCards.map((card) => (
              <div key={card.id} className="rounded-2xl bg-slate-50 p-3">
                <p className="text-sm font-black">{card.name}</p>
                <p className="mt-1 text-xs font-bold leading-5 text-slate-500">저장하지 않고 넘긴 카드가 다시 조건을 충족했습니다.</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <button onClick={() => { logEvent('result_view', { source: 'manual_update' }); showToast('저장한 카드 결과를 다시 확인했습니다.'); }} className="mx-5 flex w-[calc(100%-40px)] items-center justify-between rounded-2xl border border-blue-200 bg-blue-50 p-4 text-left text-base font-black text-[#0B63F6]">
          저장한 카드 결과 다시 보기
