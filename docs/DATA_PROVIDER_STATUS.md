...
| Kiwoom REST after-hours | KR after-hours single price candidate | `KIWOOM_APP_KEY`/`KIWOOM_SECRET_KEY` or legacy Kiwoom env | partial, route implemented | 2026-05-14 build verified | Detail diagnosis/Why appeared | Selected Kiwoom `ka10087` as the lawful source candidate based on Kiwoom REST endpoint catalogs. Cron cadence: EOD 18:20~19:00 KST once. Production response format still needs validation with active Kiwoom env. |
